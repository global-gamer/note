---
{"dg-publish":true,"phase":5,"period":"Jun–Jul 2026","focus":"Public launch, marketing momentum, supporter rollout","primary_owner":"Growth Director","secondary_owner":"Partnerships Manager","status":"upcoming","status_icon":"⏳","milestones":["Launch Event","Paid Campaign Bundle","Supporter Guild Tiers"],"permalink":"/05-phasenplan/phase-5-public-launch/","dgPassFrontmatter":true,"noteIcon":"","created":"2025-12-08T18:29:56.862+01:00","updated":"2025-12-13T16:05:45.661+01:00"}
---


# Phase 5 – Public Launch & Event Momentum

**Ziele:** Offizieller Launch, 150 aktive Member, laufender Event-Kalender mit Marketing-Unterstützung.

## Key Workstreams
- Launch Event (Stream + Forum/Discord Watch Party) → Creator Alliance einbinden.
- Paid/Owned Media Kampagnen (Discord Ads, Reddit, Micro-Influencer).
- Website Relaunch mit Social Proof, Live KPIs, Partner CTA.
- Node02 Provisioning vorbereiten (bei Auslastung >75 %).

## Economic Focus
- Sponsoring Deck finalisieren, 5–8 warme Leads ansprechen.
- Supporter Guild Angebot veröffentlichen (3 Tiers, Benefits, Automationen).

## Check / KPIs
- 150 aktive Member, mind. 3 Events/Monat (≥25 Teilnehmer).
- 2 Sponsoring Leads im Pitch-Status, >1.000€ monatliche Supporter-Einnahmen in Aussicht.
- Website Conversion (Visitor → Discord) ≥40 %.

## Milestones & Owners
| Deliverable | Owner |
| --- | --- |
| Public Launch Event (Stream + Watch Parties) | Event Director + Growth Director |
| Marketing Campaign Bundle (Discord Ads, Reddit, Influencer) | Growth Manager |
| Website Relaunch mit Live KPIs & Partner CTA | Web Lead |
| Node02 Procurement & Readiness Checklist | Operations Director |
| Supporter Guild Tier Launch (3 Tiers) | Merch & VIP Manager |
| Sponsorship Outreach (8 Leads) | Partnerships Manager |

## Notes
- Track Launch KPIs im Data Studio Dashboard + Screenshot in Meeting Notes.
- Evaluiere Node02 Implementation nach Launch load KPIs (discord growth, server load).

↩ [[Home\|Home]] · Nächste Phase: [[05_Phasenplan/Phase 6 Stabilize Automate\|Phase 6]]
